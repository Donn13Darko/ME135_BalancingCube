...
/*******************************************************************************
* Function Name: accel_clock_Start
********************************************************************************
*
* Summary:
...
void accel_clock_Start(void) 
{
    ...
    accel_clock_CLKEN |= accel_clock_CLKEN_MASK;
	accel_clock_CLKSTBY |= accel_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: accel_clock_Stop
********************************************************************************
*
* Summary:
...
void accel_clock_Stop(void) 
{
    ...
    accel_clock_CLKEN &= (uint8)(~accel_clock_CLKEN_MASK);
	accel_clock_CLKSTBY &= (uint8)(~accel_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: accel_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((accel_clock_CLKEN & accel_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = accel_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = accel_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* accel_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(accel_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(accel_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: accel_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: accel_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = accel_clock_GetSourceRegister();
    uint16 oldDivider = accel_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = accel_clock_CLKEN & accel_clock_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((accel_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: accel_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(accel_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: accel_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    accel_clock_MOD_SRC |= modeBitMask & (uint8)accel_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: accel_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    accel_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(accel_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: accel_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return accel_clock_MOD_SRC & (uint8)(accel_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: accel_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = accel_clock_GetDividerRegister();
    uint8 oldSrc = accel_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        accel_clock_MOD_SRC |= CYCLK_SSS;
        accel_clock_MOD_SRC =
            (accel_clock_MOD_SRC & (uint8)(~accel_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: accel_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return accel_clock_MOD_SRC & accel_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: accel_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    accel_clock_PHASE = clkPhase & accel_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: accel_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return accel_clock_PHASE & accel_clock_PHASE_MASK;
}
