...
uint8 i2c_COM_initVar = 0u; /* Defines if component was initialized */

volatile uint8 i2c_COM_state;  /* Current state of I2C FSM */


/*******************************************************************************
* Function Name: i2c_COM_Init
********************************************************************************
*
* Summary:
...
void i2c_COM_Init(void) 
{
    ...
    i2c_COM_CFG_REG  = i2c_COM_DEFAULT_CFG;
    i2c_COM_XCFG_REG = i2c_COM_DEFAULT_XCFG;
    i2c_COM_ADDR_REG = i2c_COM_DEFAULT_ADDR;
    i2c_COM_CLKDIV1_REG = LO8(i2c_COM_DEFAULT_DIVIDE_FACTOR);
    i2c_COM_CLKDIV2_REG = HI8(i2c_COM_DEFAULT_DIVIDE_FACTOR);
    ...
    i2c_COM_CFG_REG      = i2c_COM_DEFAULT_CFG;
    i2c_COM_INT_MASK_REG = i2c_COM_DEFAULT_INT_MASK;

    /* Enable interrupt generation in status */
    intState = CyEnterCriticalSection();
    i2c_COM_INT_ENABLE_REG |= i2c_COM_INTR_ENABLE;
    CyExitCriticalSection(intState);

    /* Configure bit counter */
    #if (i2c_COM_MODE_SLAVE_ENABLED)
        i2c_COM_PERIOD_REG = i2c_COM_DEFAULT_PERIOD;
    #endif  /* (i2c_COM_MODE_SLAVE_ENABLED) */

    /* Configure clock generator */
    #if (i2c_COM_MODE_MASTER_ENABLED)
        i2c_COM_MCLK_PRD_REG = i2c_COM_DEFAULT_MCLK_PRD;
        i2c_COM_MCLK_CMP_REG = i2c_COM_DEFAULT_MCLK_CMP;
    ...
    CyIntDisable    (i2c_COM_ISR_NUMBER);
    CyIntSetPriority(i2c_COM_ISR_NUMBER, i2c_COM_ISR_PRIORITY);
    #if (i2c_COM_INTERN_I2C_INTR_HANDLER)
        (void) CyIntSetVector(i2c_COM_ISR_NUMBER, &i2c_COM_ISR);
    #endif /* (i2c_COM_INTERN_I2C_INTR_HANDLER) */
    ...
    i2c_COM_SlaveSetAddress(i2c_COM_DEFAULT_ADDR);
#endif /* (i2c_COM_MODE_SLAVE_ENABLED) */

#if (i2c_COM_MODE_MASTER_ENABLED)
    /* Clear status and buffers index */
    i2c_COM_mstrStatus = 0u;
...
void i2c_COM_Enable(void) 
{
    ...
    intState = CyEnterCriticalSection();
    i2c_COM_ACT_PWRMGR_REG  |= i2c_COM_ACT_PWR_EN;
    i2c_COM_STBY_PWRMGR_REG |= i2c_COM_STBY_PWR_EN;
    CyExitCriticalSection(intState);
#else
    #if (i2c_COM_MODE_SLAVE_ENABLED)
        /* Enable bit counter */
        uint8 intState = CyEnterCriticalSection();
        i2c_COM_COUNTER_AUX_CTL_REG |= i2c_COM_CNT7_ENABLE;
        CyExitCriticalSection(intState);
    #endif /* (i2c_COM_MODE_SLAVE_ENABLED) */

    /* Enable slave or master bits */
    i2c_COM_CFG_REG |= i2c_COM_ENABLE_MS;
#endif /* (i2c_COM_FF_IMPLEMENTED) */
...
*
* Global variables:
*  i2c_COM_initVar - This variable is used to check the initial
...
{
    if (0u == i2c_COM_initVar)
    {
        i2c_COM_Init();
        i2c_COM_initVar = 1u; /* Component initialized */
    }
...
void i2c_COM_Stop(void) 
{
...
#if (i2c_COM_TIMEOUT_ENABLED)
    i2c_COM_TimeoutStop();
#endif  /* End (i2c_COM_TIMEOUT_ENABLED) */
...
#if (i2c_COM_FF_IMPLEMENTED)
    {
        ...
        blockResetCycles = ((uint16) ((uint16) i2c_COM_CLKDIV2_REG << 8u) | i2c_COM_CLKDIV1_REG) + 1u;

        /* Disable block */
        i2c_COM_CFG_REG &= (uint8) ~i2c_COM_CFG_EN_SLAVE;
        ...
        /* Enable block */
        i2c_COM_CFG_REG |= (uint8) i2c_COM_ENABLE_MS;
...
#if (i2c_COM_MODE_SLAVE_ENABLED)
    {
        /* Disable bit counter */
        uint8 intState = CyEnterCriticalSection();
        i2c_COM_COUNTER_AUX_CTL_REG &= (uint8) ~i2c_COM_CNT7_ENABLE;
        CyExitCriticalSection(intState);
    }
#endif /* (i2c_COM_MODE_SLAVE_ENABLED) */

    /* Clear interrupt source register */
    (void) i2c_COM_CSR_REG;
#endif /* (i2c_COM_FF_IMPLEMENTED) */
    ...
    /* Clear busy statuses */
#if (i2c_COM_MODE_SLAVE_ENABLED)
    i2c_COM_slStatus &= (uint8) ~(i2c_COM_SSTAT_RD_BUSY | i2c_COM_SSTAT_WR_BUSY);
#endif /* (i2c_COM_MODE_SLAVE_ENABLED) */
}
