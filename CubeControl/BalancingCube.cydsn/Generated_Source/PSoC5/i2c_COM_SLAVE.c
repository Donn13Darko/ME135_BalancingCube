...
volatile uint8 i2c_COM_slStatus;   /* Slave Status  */

/* Transmit buffer variables */
volatile uint8 * i2c_COM_slRdBufPtr;   /* Pointer to Transmit buffer  */
volatile uint8   i2c_COM_slRdBufSize;  /* Slave Transmit buffer size  */
volatile uint8   i2c_COM_slRdBufIndex; /* Slave Transmit buffer Index */

/* Receive buffer variables */
volatile uint8 * i2c_COM_slWrBufPtr;   /* Pointer to Receive buffer  */
volatile uint8   i2c_COM_slWrBufSize;  /* Slave Receive buffer size  */
volatile uint8   i2c_COM_slWrBufIndex; /* Slave Receive buffer Index */

#if (i2c_COM_SW_ADRR_DECODE)
    volatile uint8 i2c_COM_slAddress;  /* Software address variable */
#endif /* (i2c_COM_SW_ADRR_DECODE) */


/*******************************************************************************
* Function Name: i2c_COM_SlaveStatus
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  i2c_COM_slStatus - The global variable used to store a current
*                              status of the I2C slave.
*
*******************************************************************************/
uint8 i2c_COM_SlaveStatus(void) 
{
    return (i2c_COM_slStatus);
}


/*******************************************************************************
* Function Name: i2c_COM_SlaveClearReadStatus
********************************************************************************
*
* Summary:
*  Clears the read status flags and returns they values.
*  The i2c_COM_SSTAT_RD_BUSY flag is not effected by clear.
*
* Parameters:
*  None.
*
...
uint8 i2c_COM_SlaveClearReadStatus(void) 
{
    ...
    status = (i2c_COM_slStatus & i2c_COM_SSTAT_RD_MASK);
    i2c_COM_slStatus &= (uint8) ~i2c_COM_SSTAT_RD_CLEAR;
...
/*******************************************************************************
* Function Name: i2c_COM_SlaveClearWriteStatus
********************************************************************************
*
* Summary:
*  Clears the write status flags and returns they values.
*  The i2c_COM_SSTAT_WR_BUSY flag is not effected by clear.
*
* Parameters:
*  None.
*
...
uint8 i2c_COM_SlaveClearWriteStatus(void) 
{
    ...
    /* Mask of transfer complete flag and Error status */
    status = (i2c_COM_slStatus & i2c_COM_SSTAT_WR_MASK);
    i2c_COM_slStatus &= (uint8) ~i2c_COM_SSTAT_WR_CLEAR;
...
/*******************************************************************************
* Function Name: i2c_COM_SlaveSetAddress
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  i2c_COM_Address  - The global variable used to store an I2C slave
...
{
#if (i2c_COM_SW_ADRR_DECODE)
    i2c_COM_slAddress = (address & i2c_COM_SLAVE_ADDR_MASK);
#else
    i2c_COM_ADDR_REG  = (address & i2c_COM_SLAVE_ADDR_MASK);
#endif /* (i2c_COM_SW_ADRR_DECODE) */
}


/*******************************************************************************
* Function Name: i2c_COM_SlaveInitReadBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  i2c_COM_slRdBufPtr   - The global variable used to store a pointer
*                                  to the slave read buffer.
*  i2c_COM_slRdBufSize  - The global variable used to store a slave
*                                  read buffer size.
     ...
     
{
    if (NULL != rdBuf)
    {
        ...
        /* Set buffer pointer */
        i2c_COM_slRdBufPtr   = (volatile uint8 *) rdBuf;
        i2c_COM_slRdBufSize  = bufSize;    /* Set buffer size     */
        i2c_COM_slRdBufIndex = 0u;         /* Clears buffer index */
...
/*******************************************************************************
* Function Name: i2c_COM_SlaveInitWriteBuf
********************************************************************************
*
* Summary:
*  Sets the buffer pointer and size of the read buffer. This function also
...
*
* Global variables:
*  i2c_COM_slWrBufPtr   - The global variable used to store a pointer
*                                  to the slave write buffer.
*  i2c_COM_slWrBufSize  - The global variable used to store a slave
*                                  write buffer size.
*  i2c_COM_slWrBufIndex - The global variable used to store a current
     ...
     
{
    if (NULL != wrBuf)
    {
        i2c_COM_DisableInt(); /* Lock from interrupt */
         ...
         /* Set buffer pointer */
        i2c_COM_slWrBufPtr   = (volatile uint8 *) wrBuf;
        i2c_COM_slWrBufSize  = bufSize;    /* Set buffer size     */
        i2c_COM_slWrBufIndex = 0u;         /* Clears buffer index */
...
/*******************************************************************************
* Function Name: i2c_COM_SlaveGetReadBufSize
********************************************************************************
*
* Summary:
...
{
    return (i2c_COM_slRdBufIndex);
}


/*******************************************************************************
* Function Name: i2c_COM_SlaveGetWriteBufSize
********************************************************************************
*
* Summary:
...
{
    return (i2c_COM_slWrBufIndex);
}


/*******************************************************************************
* Function Name: i2c_COM_SlaveClearReadBuf
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: i2c_COM_SlaveClearRxBuf
********************************************************************************
*
* Summary:
