...
/*******************************************************************************
* Function Name: flywheel_clock_Start
********************************************************************************
*
* Summary:
...
void flywheel_clock_Start(void) 
{
    ...
    flywheel_clock_CLKEN |= flywheel_clock_CLKEN_MASK;
	flywheel_clock_CLKSTBY |= flywheel_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: flywheel_clock_Stop
********************************************************************************
*
* Summary:
...
void flywheel_clock_Stop(void) 
{
    ...
    flywheel_clock_CLKEN &= (uint8)(~flywheel_clock_CLKEN_MASK);
	flywheel_clock_CLKSTBY &= (uint8)(~flywheel_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: flywheel_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((flywheel_clock_CLKEN & flywheel_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = flywheel_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = flywheel_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* flywheel_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(flywheel_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(flywheel_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: flywheel_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: flywheel_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = flywheel_clock_GetSourceRegister();
    uint16 oldDivider = flywheel_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = flywheel_clock_CLKEN & flywheel_clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(flywheel_clock_DIV_PTR, clkDivider);
                flywheel_clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                flywheel_clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(flywheel_clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((flywheel_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: flywheel_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(flywheel_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: flywheel_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    flywheel_clock_MOD_SRC |= modeBitMask & (uint8)flywheel_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: flywheel_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    flywheel_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(flywheel_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: flywheel_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return flywheel_clock_MOD_SRC & (uint8)(flywheel_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: flywheel_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = flywheel_clock_GetDividerRegister();
    uint8 oldSrc = flywheel_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        flywheel_clock_MOD_SRC |= CYCLK_SSS;
        flywheel_clock_MOD_SRC =
            (flywheel_clock_MOD_SRC & (uint8)(~flywheel_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: flywheel_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return flywheel_clock_MOD_SRC & flywheel_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: flywheel_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    flywheel_clock_PHASE = clkPhase & flywheel_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: flywheel_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return flywheel_clock_PHASE & flywheel_clock_PHASE_MASK;
}
